...
export default {
  ...
  methods: {
    ...
    getPage() {
      ...
      const pageRequestUrl = `https://graph.facebook.com/${
        this.userId
      }/accounts?fields=name,access_token&access_token=${this.userToken}`;
      console.log(pageRequestUrl);
      axios.get(pageRequestUrl).then(response => {
        this.info = response;
    ...
    async addPage(page) {
      ...
      if (check.length > 1) {
        ...
        const update = await db.pages.update(
          { id: page.id },
          { $set: { pageToken: page.access_token } }
        );
        console.log(update);
      } else {
