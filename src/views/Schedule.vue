  ...
  methods: {
    ...
    sendPost(site) {
      if (site === "facebook") {
        ...
        let pageRequestUrl = `https://graph.facebook.com/${
          this.selectedPage.id
        }/feed?published=false&message=${
          this.postBody
        }&scheduled_publish_time=${date}&access_token=${
          this.selectedPage.pageToken
        }`;

        axios.post(pageRequestUrl).then(response => {
          this.info = response;
